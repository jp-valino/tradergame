        ...
        <div className="p-4 border-b border-color-border">
          <div className="flex justify-between items-center">
            ...
            <div className="chart-toggle-buttons">
              <button 
                className={`chart-toggle-button ${chartView === 'normalized' ? 'active' : ''}`}
                onClick={() => setChartView('normalized')}
              >
                Normalized
              </button>
              <button 
                className={`chart-toggle-button ${chartView === 'price' ? 'active' : ''}`}
              ...
              <div className="grid grid-cols-1 gap-4">
                {stocksBySector[sector].map(stock => (
                  <div key={stock.symbol} className="flex items-center">
                    ...
                    <div className="ml-4">
                      <button
                        className={`comparison-button ${selectedStocks.includes(stock.symbol) ? 'active' : ''}`}
                        onClick={() => toggleStockSelection(stock.symbol)}
                        title={selectedStocks.includes(stock.symbol) ? 'Remove from comparison' : 'Add to comparison'}
                      >
                        <span className="icon">{selectedStocks.includes(stock.symbol) ? '📊' : '+'}</span>
                        <span className="label">{selectedStocks.includes(stock.symbol) ? 'Remove from Comparison' : 'Add to Comparison'}</span>
                      </button>
                    </div>
                  </div>
                ))}
